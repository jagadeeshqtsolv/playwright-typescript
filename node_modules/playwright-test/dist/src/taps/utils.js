...
export const IS_ENV_WITH_DOM = typeof window === 'object' &&
...
export const milli = (/** @type {number[]} */ arr) => (arr[0] * 1e3 + arr[1] / 1e6).toFixed(2) + 'ms';
/** @type {(now?: [number, number]) => () => string} */
let _hrtime = (now = [Date.now(), 0]) => () => (Date.now() - now[0]).toFixed(2) + 'ms';
if (IS_NODE && 'hrtime' in process) {
    _hrtime =
        (now = process.hrtime()) => () => milli(process.hrtime(now));
}
if ('performance' in globalThis && 'now' in globalThis.performance) {
    _hrtime =
        (now = [performance.now(), 0]) => () => (performance.now() - now[0]).toFixed(2) + 'ms';
}
